...
metadata_files = sorted(glob.glob(path + "/DT_datMeta*"+".csv"))
expression_files = sorted(glob.glob(path + "/DT_datExpr*"+".csv"))

def get_dataset_name(file_path):
    #namee = os.path.basename(file_path).replace("datExpr.HTSC.unionexon.", "").replace(".filtered.csv", "")
    namee = os.path.basename(file_path).replace("DT_datMeta_", "").replace("DT_datExpr_", "").replace("_unionexon.csv", "")
