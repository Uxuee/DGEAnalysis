...
rm(create_DT)
## Lets create the second dataset: DT
create_DT <- function(expr_matrix_A, expr_matrix_B, meta_A, meta_B) {

    # Step 1: Find subjects present in both tissues
    common_ids <- intersect(meta_A$BrainID, meta_B$BrainID)

    # Step 2: Keep only common subjects
    meta_A <- meta_A[meta_A$BrainID %in% common_ids, ]
    meta_B <- meta_B[meta_B$BrainID %in% common_ids, ]

    # Step 3: Sort by BrainID to align
    meta_A <- meta_A[order(meta_A$BrainID), ]
    meta_B <- meta_B[order(meta_B$BrainID), ]

    # Step 4: Check matching BrainIDs
    stopifnot(all(meta_A$BrainID == meta_B$BrainID))
  
    # Step 5: Intersect genes
    intersect_genes <- intersect(rownames(expr_matrix_A), rownames(expr_matrix_B))
    # Subset expression matrices to only include intersecting genes
    expr_matrix_A <- expr_matrix_A[intersect_genes, ]
    expr_matrix_B <- expr_matrix_B[intersect_genes, ]

    # Step 5: Subset expression matrices
    expr_A <- expr_matrix_A[, rownames(meta_A)]
    expr_B <- expr_matrix_B[, rownames(meta_B)]
  
    # Step 6: Calculate the DT (Tissue A - Tissue B)
    DT <- expr_A - expr_B

    # Step 7: Rename columns to BrainIDs
    colnames(DT) <- meta_A$BrainID

    rownames(meta_A)<-meta_A$BrainID
    head(meta_A)

    # Step 8: Return DT
    return(list("DT"=DT,"meta"=meta_A))
}

# Create DT matrices for each dataset
# F = ba9 (Frontal), T = ba41-42-22 (Temporal), C = vermis (Cerebellum)
#: C-T, C-F, F-T.
